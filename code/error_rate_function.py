...
def dd_error_rate(fp_dd, fp_mta, ecv_):
    print(fp_dd)
    dd_er_ = []
    for ecv in ecv_:
    ...
    return np.array(dd_er_, dtype=np.float32)

def mta_error_rate(fp_mta, ecv_):
    mta_er_ = []
    for ecv in ecv_:
        ...
        mta_er_.append(mta_er)
    
    return np.array(mta_er_, dtype=np.float32)
    

def error_rate(ecv, d0,d1,d2,d3,d4,d5):
    fp_dd = [d0, d1, d2, d3, d4, d5]
    fp_mta = [0.3, 0.088, 0.042, 319.2]
